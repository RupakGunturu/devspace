...
export const StaggeredMenu = forwardRef<StaggeredMenuHandle, StaggeredMenuProps>(function StaggeredMenu({
  ...
  }, [playOpen, playClose, animateIcon, animateColor, animateText, onMenuOpen, onMenuClose, hamburger]);
  ...
  }, [playClose, animateIcon, animateColor, animateText, onMenuClose, hamburger]);

  useImperativeHandle(ref, () => ({
    open: () => { if (!openRef.current) toggleMenu(); },
    close: () => { if (openRef.current) closeMenu(); },
    toggle: toggleMenu,
  ...
  useLayoutEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isOutsideToggle = hamburger || (toggleBtnRef.current && !toggleBtnRef.current.contains(event.target as Node));
  ...
  }, [closeOnClickAway, open, closeMenu, hamburger]);

  return (
    <div
      className={(className ? className + " " : "") + "staggered-menu-wrapper" + (isFixed ? " fixed-wrapper" : "") + (hamburger ? " hamburger-mode" : "")}
        ...
        <div className="sm-panel-inner">
          {hamburger && (
            <div className="sm-panel-header">
              <span className="sm-panel-logo font-display text-2xl font-extrabold text-text">
                dev<span className="text-yellow">/</span>space
              </span>
              <button className="sm-panel-close" onClick={closeMenu} aria-label="Close menu" type="button">
