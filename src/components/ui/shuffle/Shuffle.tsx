import { useRef, useEffect, useState, useMemo, type CSSProperties, type ElementType } from "react";
...
const Shuffle = ({
  ...
  const hoverHandlerRef = useRef<((evt: MouseEvent) => void) | null>(null);
  ...
  useGSAP(
    () => {
      ...
      const inners = () =>
        wrappersRef.current.map((w) => w.firstElementChild).filter((x): x is Element => x !== null);
      ...
      const play = () => {
        ...
        const addTween = (targets: Element[], at?: number) => {
          ...
          if (isVertical) {
            vars.y = (i: number, t: Element) => parseFloat(t.getAttribute("data-final-y") || "0");
          } else {
            vars.x = (i: number, t: Element) => parseFloat(t.getAttribute("data-final-x") || "0");
          }
