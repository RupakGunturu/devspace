...
const SPAM_TRIGGERS = [
  "free", "winner", "winner!!!", "congratulations", "act now", "urgent",
  "limited time", "exclusive deal", "no cost", "risk free", "no risk",
  "buy now", "order now", "click here", "subscribe now", "100% free",
  "amazing offer", "best price", "cash bonus", "credit", "discount",
  "do it today", "don't delete", "don't hesitate", "fantastic deal",
  "for free", "guarantee", "hurry", "immediate", "info you requested",
  "insurance", "lowest price", "million dollars", "money back",
  "no catch", "no obligation", "no strings attached", "obligation",
  "offer expires", "once in a lifetime", "pennies", "promise",
  "pure profit", "satisfaction", "special promotion", "take action",
  "this isn't spam", "trial", "unlimited", "what are you waiting for",
  "while supplies last", "you have been selected", "you're a winner",
];

const URGENCY_WORDS = [
  "now", "today", "hurry", "limited", "expires", "deadline", "last chance",
  "final", "ending soon", "only today", "don't miss", "before it's gone",
  "running out", "almost gone", "countdown", "last call",
];
...
export function EmailSubjectTester() {
  ...
  const checks = analysis
    ? [
        { label: "Length", value: `${analysis.len} chars`, ok: analysis.len >= 20 && analysis.len <= 60, detail: "Optimal: 20-60 chars" },
        { label: "Words", value: `${analysis.wordCount}`, ok: analysis.wordCount >= 3 && analysis.wordCount <= 15, detail: "Optimal: 3-15 words" },
        { label: "Spam Words", value: `${analysis.foundSpam.length} found`, ok: analysis.foundSpam.length === 0, detail: analysis.foundSpam.length > 0 ? analysis.foundSpam.join(", ") : "None detected" },
        { label: "Personalization", value: `${analysis.foundPersonalization.length > 0 ? "Yes" : "No"}`, ok: analysis.foundPersonalization.length > 0, detail: analysis.foundPersonalization.length > 0 ? analysis.foundPersonalization.join(", ") : "Add %name% or {{first_name}}" },
        { label: "Urgency Words", value: `${analysis.foundUrgency.length}`, ok: analysis.foundUrgency.length >= 1 && analysis.foundUrgency.length <= 2, detail: analysis.foundUrgency.length > 0 ? analysis.foundUrgency.join(", ") : "Add urgency sparingly" },
        { label: "Emoji Count", value: `${analysis.emojiCount}`, ok: analysis.emojiCount >= 0 && analysis.emojiCount <= 1, detail: "0-1 emoji recommended" },
        { label: "Question Mark", value: subject.trim().endsWith("?") ? "Yes" : "No", ok: subject.trim().endsWith("?"), detail: "Questions boost opens" },
      ]
    : [];
    ...
    <ToolLayout id="email-subject-tester">
      ...
      {analysis && (
        <div className="rounded-md border-2 border-line bg-input-bg p-6 text-center">
          ...
          >
            <span className="font-mono text-3xl font-bold" style={{ color: scoreColor(analysis.score) }}>
              {analysis.score}
            </span>
          </div>
          <div className="mt-3 font-mono text-sm font-medium" style={{ color: scoreColor(analysis.score) }}>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs" style={{ color: c.ok ? "#22c55e" : "#ef4444" }}>
                    {c.value}
                  </span>
                </div>
        ...
        <div className="rounded-md border-2 border-line bg-input-bg p-4">
          ...
          <div className="border-b border-line pb-3">
            ...
            <div className="mt-1 font-mono text-xs text-muted">
              Your Brand — This is a preview of how your subject line will appear in email inboxes...
            </div>
          </div>
        </div>
      )}
