...
export function PollSurveyBuilder() {
  ...
  const addOption = () => {
    ...
    setQuestions((prev) =>
      prev.map((q) =>
        q.id === activeQ
          ? { ...q, options: [...q.options, { id: nextOId++, text: newOption.trim(), votes: 0 }] }
          : q,
      ),
    );
    setNewOption("");
  ...
  const removeQuestion = (id: number) => {
    ...
    if (activeQ === id)
      setActiveQ(questions.length > 1 ? questions.find((q) => q.id !== id)!.id : null);
  };
    ...
    setQuestions((prev) =>
      prev.map((q) =>
        q.id === qId
          ? {
              ...q,
              options: q.options.map((o) => (o.id === oId ? { ...o, votes: o.votes + 1 } : o)),
            }
          : q,
      ),
    );
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            className="w-full rounded-md border-2 border-line bg-input-bg p-2.5 font-mono text-sm text-input-text outline-none placeholder:text-muted"
            style={{ borderColor: newQuestion ? color : undefined }}
          />
        </div>
        <div className="flex items-end">
          <ToolButton onClick={addQuestion} disabled={!newQuestion.trim()}>
            Add Question
          </ToolButton>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-muted transition-colors hover:text-coral font-mono text-xs"
            >
              \u00d7 Remove
            </button>
          </div>
          ...
          <div className="flex gap-2">
            ...
            />
            <ToolButton onClick={addOption} disabled={!newOption.trim()}>
              Add Option
            </ToolButton>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-[10px] text-muted">
                          {opt.votes} votes ({pct.toFixed(0)}%)
                        </span>
                        <button
                          onClick={() => removeOption(activeQ!, opt.id)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => addVote(activeQ!, opt.id)}
                        className="shrink-0 rounded-md border-2 px-2 py-0.5 font-mono text-[10px] transition-all hover:opacity-80"
          ...
          {totalVotes > 0 && (
            <div className="flex items-center justify-between rounded-md border-2 border-line bg-input-bg px-3 py-2">
              ...
              <CopyButton
                text={activeQuestion.options
                  .map(
                    (o) =>
                      `${o.text}: ${o.votes} (${totalVotes > 0 ? ((o.votes / totalVotes) * 100).toFixed(0) : 0}%)`,
                  )
                  .join("\n")}
              />
            </div>
