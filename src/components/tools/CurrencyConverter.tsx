  ...
  return (
    <ToolLayout id="currency-converter">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-end">
        <div>
          <span className="mb-1 block font-mono text-xs font-medium uppercase tracking-wider text-muted">From</span>
          <select
            value={from}
        ...
        <div>
          <span className="mb-1 block font-mono text-xs font-medium uppercase tracking-wider text-muted">To</span>
          <select
            value={to}
      ...
      {converted !== null && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-md border-2 border-line bg-input-bg p-4">
            <div className="font-mono text-xs uppercase tracking-wider text-muted">Exchange Rate</div>
          ...
          <div className="rounded-md border-2 border-line bg-input-bg p-4">
            <div className="font-mono text-xs uppercase tracking-wider text-muted">Conversion Summary</div>
            <div className="mt-1 font-mono text-sm text-input-text">
              {fmt(parseFloat(amount) || 0)} {from} = {fmt(converted)} {to}
            </div>
            <div className="mt-0.5 font-mono text-xs text-muted">
              {getInput(from)?.symbol}{fmt(parseFloat(amount) || 0)} \u2192 {getInput(to)?.symbol}{fmt(converted)}
            </div>
          </div>
        </div>
      )}
      ...
      <div>
        <span className="mb-2 block font-mono text-xs font-medium uppercase tracking-wider text-muted">Quick Reference</span>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[1, 10, 100, 1000].map((val) => {
            const inUsd = val / RATES[from];
            const result = inUsd * RATES[to];
            return (
              <div key={val} className="rounded-md border-2 border-line bg-input-bg p-2 text-center">
                <div className="font-mono text-xs text-muted">{val} {from}</div>
                <div className="font-mono text-sm font-bold" style={{ color }}>{fmt(result)} {to}</div>
              </div>
            );
          })}
        </div>
