...
export function CommissionCalculator() {
  ...
  const addTier = () => {
    const lastMax =
      tiers.length > 0 ? tiers[tiers.length - 1].max || tiers[tiers.length - 1].min + 50000 : 0;
    setTiers((prev) => [
      ...prev,
      { id: nextTierId++, min: lastMax, max: lastMax + 50000, rate: rate || 10 },
    ]);
  };
    ...
    <ToolLayout id="commission-calculator">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            style={{ borderColor: baseSalary ? color : undefined }}
          />
        </div>
        <div>
          <label className="mb-2 block font-mono text-xs font-medium uppercase tracking-wider text-muted">
          ...
          <input
            ...
            placeholder="10"
            className="w-full rounded-md border-2 border-line bg-input-bg p-3 font-mono text-sm text-input-text outline-none placeholder:text-muted"
            style={{ borderColor: commissionRate ? color : undefined }}
          />
        </div>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            placeholder="500000"
            className="w-full rounded-md border-2 border-line bg-input-bg p-3 font-mono text-sm text-input-text outline-none placeholder:text-muted"
            style={{ borderColor: target ? color : undefined }}
          />
        </div>
        <div>
          <label className="mb-2 block font-mono text-xs font-medium uppercase tracking-wider text-muted">
      ...
      {useTiers && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <ToolButton variant="secondary" onClick={addTier}>
              + Add Tier
            </ToolButton>
          </div>
          {tiers.map((tier) => (
            ...
            >
              ...
              <input
                ...
                className="w-24 bg-transparent font-mono text-xs text-input-text outline-none"
              />
              ...
              <input
                ...
                className="w-16 bg-transparent font-mono text-xs text-input-text outline-none"
              />
              <span className="font-mono text-xs text-muted">%</span>
              <button
                onClick={() => removeTier(tier.id)}
                className="ml-auto text-muted transition-colors hover:text-coral"
              >
                \u00d7
              </button>
            </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-lg border-2 border-line bg-input-bg p-3 text-center">
          ...
          <div className="font-display text-lg font-extrabold" style={{ color: "#10b981" }}>
            ${fmt(totalComp)}
          </div>
        </div>
        <div className="rounded-lg border-2 border-line bg-input-bg p-3 text-center">
          ...
          <div
            ...
            style={{ color: quotaPct >= 100 ? "#10b981" : quotaPct >= 75 ? "#f59e0b" : "#ef4444" }}
          >
            {quotaPct.toFixed(1)}%
          </div>
        </div>
      ...
      {salesTarget > 0 && (
        <div>
          <div className="mb-1 flex items-center justify-between font-mono text-xs">
            ...
            <span
              style={{
                color: quotaPct >= 100 ? "#10b981" : quotaPct >= 75 ? "#f59e0b" : "#ef4444",
              }}
            >
              {quotaPct.toFixed(1)}%
            </span>
          ...
          <div className="h-4 overflow-hidden rounded-full bg-line">
            <div
              ...
              style={{
                ...
                backgroundColor:
                  quotaPct >= 100 ? "#10b981" : quotaPct >= 75 ? "#f59e0b" : "#ef4444",
              }}
            />
          </div>
