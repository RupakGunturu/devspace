...
export default function PercentageCalculator() {
  ...
  const calculate = () => {
    ...
    switch (mode) {
      case "of":
        setResult(`${numA}% of ${numB} = ${((numA / 100) * numB).toFixed(4)}`);
        break;
      case "is":
        setResult(`${numA} is ${((numA / numB) * 100).toFixed(4)}% of ${numB}`);
        break;
      case "change":
        setResult(`Change from ${numA} to ${numB}: ${(((numB - numA) / numA) * 100).toFixed(4)}%`);
        break;
    }
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">
            {mode === "of" ? "Percentage" : mode === "is" ? "Value" : "From"}
          </label>
          <input
            type="number"
            value={a}
