...
export default function CssTextShadowBuilder() {
  ...
  const generate = () =>
    setOutput(
      `text-shadow: ${shadows.map((s) => `${s.x}px ${s.y}px ${s.blur}px ${s.color}`).join(", ")};`,
    );
    ...
    <ToolLayout id="css-text-shadow-builder">
          ...
          <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 sm:w-14 p-2 bg-paper-dim/50 border border-border rounded text-xs font-mono text-foreground"
                placeholder="X"
              />
              <input
                type="number"
      ...
      <div className="p-4 bg-paper-dim/50 border border-border rounded-sm">
        <p
          ...
          style={{
            textShadow: shadows.map((s) => `${s.x}px ${s.y}px ${s.blur}px ${s.color}`).join(", "),
          }}
        >
          Hello World
        </p>
