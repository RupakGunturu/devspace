...
export default function SqlQueryGenerator() {
  ...
  const generate = () => {
    ...
    if (
      ...
      setOutput(
        `SELECT ${field === "all" || lower.includes("all") ? "*" : field} FROM ${table}${lower.includes("where") ? " WHERE condition" : ""}${lower.includes("sort") || lower.includes("order") ? " ORDER BY created_at DESC" : ""}${lower.includes("limit") ? " LIMIT 10" : ""};`,
      );
      ...
      setOutput(
        "Try: 'get all users', 'add a new user', 'update user where id is 1', 'delete user'",
      );
  };
  ...
  return (
    <ToolLayout id="sql-query-generator">
      <ToolInput
        value={input}
        onChange={setInput}
        placeholder="get all users where active is true"
