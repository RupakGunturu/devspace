...
export default function TechStackSuggester() {
  ...
  const stacks: Record<
    string,
    { frontend: string; backend: string; database: string; deploy: string }
  > = {
    blog: {
  ...
  const suggest = () => {
    ...
    if (match) {
      const s = match[1];
      setOutput(
        `Recommended stack for "${match[0]}":\n\nFrontend: ${s.frontend}\nBackend: ${s.backend}\nDatabase: ${s.database}\nDeployment: ${s.deploy}`,
      );
    } else
      setOutput("Try describing: blog, ecommerce, social, dashboard, mobile, ai, portfolio, saas");
  };
  ...
  return (
    <ToolLayout id="tech-stack-suggester">
      <ToolInput
        value={input}
        onChange={setInput}
        placeholder="I want to build a social media app..."
