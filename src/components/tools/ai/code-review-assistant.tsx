...
export default function CodeReviewAssistant() {
  ...
  const review = () => {
    ...
    if (input.includes("console.log"))
      issues.push("⚠️  Found console.log — remove before production");
    ...
    if (/\bfunction\s+\w+\([^)]*\)\s*\{[\s\S]{500,}\}/.test(input))
      issues.push("⚠️  Long function — consider breaking into smaller functions");
