...
export default function ErrorMessageExplainer() {
  ...
  const errors: Record<string, string> = {
    "Cannot read properties of undefined":
      "You're trying to access a property on undefined. Add null checks or optional chaining (?.).",
    "is not a function":
      "You're calling something as a function but it isn't one. Check the variable type.",
    "Module not found":
      "Import path is wrong or package not installed. Check the import statement.",
    "Maximum call stack size exceeded":
      "Infinite recursion. A function is calling itself without a base case.",
    SyntaxError:
      "There's a syntax error in your code. Check for missing brackets, semicolons, or quotes.",
  ...
  const explain = () => {
    ...
    if (matches.length > 0)
      setOutput(matches.map(([key, desc]) => `${key}\n→ ${desc}`).join("\n\n"));
