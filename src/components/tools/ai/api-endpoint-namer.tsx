...
export default function ApiEndpointNamer() {
  ...
  const generate = () => {
    ...
    const resource =
      lower.match(/(user|post|comment|product|order|category|tag|message)/)?.[0] || "resource";
    const plural = resource + "s";
    if (lower.includes("get") || lower.includes("list") || lower.includes("fetch"))
      setOutput(`GET /api/${plural}\nGET /api/${plural}/:id`);
      ...
      setOutput(`PUT /api/${plural}/:id`);
    else if (lower.includes("delete") || lower.includes("remove"))
      setOutput(`DELETE /api/${plural}/:id`);
