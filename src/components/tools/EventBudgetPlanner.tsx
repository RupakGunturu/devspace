...
export function EventBudgetPlanner() {
  ...
  const addItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      { id: nextId++, category, description: description.trim(), estimated: est, actual: act },
    ]);
    setDescription("");
    setEstimated("");
    setActual("");
  ...
  const allText = items
    .map(
      (i) =>
        `${i.category} | ${i.description} | Est: $${fmt(i.estimated)} | Act: $${fmt(i.actual)} | Var: $${fmt(i.estimated - i.actual)}`,
    )
    .join("\n");
  ...
  return (
    <ToolLayout id="event-budget-planner">
          ...
          <div className="flex flex-wrap gap-1">
            {CATEGORIES.map((c) => (
              <button
                ...
                className="rounded-md border-2 px-2 py-1 font-mono text-[10px] transition-all"
                style={
                  category === c
      ...
      <div className="flex flex-wrap gap-2">
        <ToolButton onClick={addItem} disabled={!description.trim()}>
          Add Item
        </ToolButton>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border-2 border-line bg-input-bg p-3 text-center">
          ...
          <div
            ...
            style={{ color: variance >= 0 ? "#10b981" : "#ef4444" }}
          >
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="font-mono text-[10px] text-muted">
                    Est ${fmt(ct.estimated)} | Act ${fmt(ct.actual)}
                  </span>
                </div>
                {ct.estimated > 0 && (
                  <div className="h-2 overflow-hidden rounded-full bg-line">
                    <div
                      ...
                      style={{
                        width: `${Math.min((ct.actual / ct.estimated) * 100, 100)}%`,
                        backgroundColor: ct.actual <= ct.estimated ? "#10b981" : "#ef4444",
                      }}
                    />
                  </div>
                )}
