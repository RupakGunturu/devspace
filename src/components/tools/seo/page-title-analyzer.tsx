...
export default function PageTitleAnalyzer() {
  ...
  const analyze = () => {
    ...
    if (len >= 50 && len <= 60) {
      score += 25;
      details.push("✓ Perfect length (50-60 chars)");
    } else if (len >= 30 && len < 50) {
      score += 10;
      details.push("⚠ Acceptable length but could be longer");
    } else if (len > 60) {
      score -= 15;
      details.push("✗ Too long — will be truncated in SERPs");
    } else {
      score -= 10;
    ...
    if (title.includes("|") || title.includes("-") || title.includes(":")) {
      score += 10;
      details.push("✓ Contains separator");
    } else details.push("⚠ No separator (| or -)");
    if (/[A-Z]/.test(title) && /[a-z]/.test(title)) {
      score += 5;
      details.push("✓ Mixed case");
    }
