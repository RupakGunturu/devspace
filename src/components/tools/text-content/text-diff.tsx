  ...
  const diff = () => {
    ...
    for (let i = 0; i < maxLen; i++) {
      if (i >= linesA.length) r.push({ type: "added", text: linesB[i] });
        ...
        r.push({ type: "removed", text: linesA[i] });
        r.push({ type: "added", text: linesB[i] });
      }
    }
    setResult(r);
  };
      ...
      {result.length > 0 && (
        <div className="w-full font-mono text-sm border border-border rounded-sm overflow-hidden">
          {result.map((r, i) => (
            <div
              key={i}
              className={`px-4 py-0.5 border-b border-border ${r.type === "added" ? "bg-coral/10 text-green-600" : r.type === "removed" ? "bg-coral/10 text-red-600 line-through" : "text-foreground"}`}
            >
