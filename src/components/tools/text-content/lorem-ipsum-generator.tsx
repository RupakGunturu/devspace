    ...
    <ToolLayout id="lorem-ipsum-generator">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          {(["words", "sentences", "paragraphs"] as const).map((u) => <button key={u} onClick={() => setUnit(u)} className={`px-3 py-1.5 text-xs rounded-full border transition-all ${unit === u ? "bg-yellow text-white border-yellow" : "border-border text-muted-foreground"}`}>{u}</button>)}
        </div>
