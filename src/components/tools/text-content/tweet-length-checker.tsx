    ...
    <ToolLayout id="tweet-length-checker">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-sm font-mono font-bold ${remaining < 0 ? "text-coral" : remaining < 20 ? "text-yellow-500" : "text-coral"}`}
          >
            {remaining} remaining
          </span>
        </div>
        <div className="w-full h-2 bg-paper-dim rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${percent > 100 ? "bg-coral" : percent > 80 ? "bg-yellow-500" : "bg-coral"}`}
