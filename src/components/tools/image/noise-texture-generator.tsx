    ...
    <ToolLayout id="noise-texture-generator">
      ...
      {url && (
        <div className="flex flex-col items-center gap-4">
          <img src={url} alt="Noise" className="border border-border rounded-sm bg-white" />
          <div className="p-4 bg-paper-dim/50 border border-border rounded-sm w-full">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">
              CSS Usage
            </span>
            <p className="font-mono text-xs text-foreground mt-1 break-all">
              background-image: url("{url.slice(0, 50)}...");
            </p>
          </div>
        </div>
      )}
