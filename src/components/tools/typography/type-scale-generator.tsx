      ...
      <div className="flex gap-2 flex-wrap">
        {scales.map((s) => (
          <button
            ...
            className="px-3 py-1.5 text-xs rounded-full border border-border text-muted-foreground hover:border-yellow/50 transition-all"
          >
            {s.name}
          </button>
        ))}
        ...
        <div className="space-y-2">
          {output.map((s) => (
            ...
            >
              ...
              <span className="font-sans text-foreground" style={{ fontSize: s.px }}>
                {s.name}
              </span>
