  ...
  return (
    <ToolLayout id="regex-tester">
      <div className="flex flex-wrap gap-2">
        <div
          className="flex flex-1 items-center gap-1 rounded-md border-2 border-line bg-input-bg px-2 focus-within:ring-2"
      ...
      <div>
        <span className="mb-2 block font-mono text-xs font-medium uppercase tracking-wider text-muted">
          {error ? (
            <span className="text-coral">error: {error}</span>
          ) : (
        ...
        {!error && matches.length > 0 && (
          <ul className="space-y-1">
            {matches.map((m, i) => (
              <li
                key={i}
                className="rounded-md border-2 border-line bg-input-bg p-2 font-mono text-xs text-input-text"
              >
