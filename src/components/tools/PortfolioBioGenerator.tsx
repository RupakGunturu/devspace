        ...
        <div className="space-y-3">
          <div className="flex gap-2">
            {tones.map((t) => (
              <button
                ...
                className={`rounded-full border-2 px-3 py-1 text-xs transition-all ${activeTone === t ? "border-accent bg-accent text-accent-fg" : "border-line text-muted"}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="rounded-md border-2 border-line bg-input-bg p-4">
            ...
            <p className="text-sm text-input-text leading-relaxed">
              {bios[activeTone as keyof typeof bios]}
            </p>
          </div>
          <div className="text-xs text-muted">
            {bios[activeTone as keyof typeof bios].length} characters
          </div>
        </div>
      </div>
    </ToolLayout>
