  ...
  return (
    <ToolLayout id="random-color-generator">
      <div className="space-y-2">
        {[
          { label: "Hue", val: hue, set: setHue, max: 360, key: "hue" as const },
          { label: "Saturation", val: sat, set: setSat, max: 100, key: "sat" as const },
          { label: "Lightness", val: light, set: setLight, max: 100, key: "light" as const },
        ].map((f) => (
              ...
              <button
                ...
                style={
                  locked[f.key]
                    ...
                    : { backgroundColor: "var(--paper-dim)", color: "var(--muted-foreground)" }
                }
              >
                {locked[f.key] ? "🔒" : "🔓"}
              </button>
      ...
      <div className="flex gap-2">
        <ToolButton onClick={generate}>Randomize</ToolButton>
        <span className="font-mono text-sm self-center">{hex()}</span>
      </div>
    </ToolLayout>
  );
}
