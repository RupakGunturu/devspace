...
export default function ColorStoryFromEmotion() {
  ...
  const generate = () => {
    ...
    if (match) {
      setOutput(
        `Mood: ${match[0]}\n\nPalette:\n${match[1].map((c) => `${c}  ████`).join("\n")}\n\nCSS: ${match[1].map((c, i) => `--color-${i + 1}: ${c};`).join("\n")}`,
      );
    } else
      setOutput(
        "Try: happy, sad, energetic, calm, mysterious, professional, creative, nature, luxury",
      );
  };

  return (
    <ToolLayout id="color-story-from-emotion">
      <ToolInput
        value={mood}
        onChange={setMood}
