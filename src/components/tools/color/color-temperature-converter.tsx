...
export default function ColorTemperatureConverter() {
  ...
  const convert = () => {
    ...
    if (isNaN(k) || k < 1000 || k > 40000) {
      setOutput("Enter Kelvin value (1000-40000)");
      return;
    }
    const temp = k / 100;
    const r =
      temp <= 66
        ? 255
        : Math.max(0, Math.min(255, 329.698727446 * Math.pow(temp - 60, -0.1332047592)));
    const g =
      temp <= 66
