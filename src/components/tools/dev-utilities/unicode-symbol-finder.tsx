...
export default function UnicodeSymbolFinder() {
  ...
  const symbols = [
    ...
    { char: "☆", name: "Star Outline", code: "U+2606" },
    { char: "♥", name: "Heart", code: "U+2665" },
    { card: "♦", char: "◆", name: "Diamond", code: "U+25C6" },
    { char: "♠", name: "Spade", code: "U+2660" },
    { char: "♣", name: "Club", code: "U+2663" },
    { char: "✓", name: "Checkmark", code: "U+2713" },
    { char: "✗", name: "Cross", code: "U+2717" },
    { char: "⚡", name: "Lightning", code: "U+26A1" },
    { char: "☕", name: "Coffee", code: "U+2615" },
    { char: "🔥", name: "Fire", code: "U+1F525" },
    { char: "💡", name: "Bulb", code: "U+1F4A1" },
  ];
  ...
  const find = () => {
    ...
    setOutput(
      results.length
        ? results.map((s) => `${s.char}  ${s.name}  ${s.code}`).join("\n")
        : "No symbols found",
    );
  };
  ...
  return (
    <ToolLayout id="unicode-symbol-finder">
      <ToolInput
        value={search}
        onChange={setSearch}
        placeholder="Search by name or code (e.g. arrow, heart, pi)"
        label="Search"
        rows={1}
      />
