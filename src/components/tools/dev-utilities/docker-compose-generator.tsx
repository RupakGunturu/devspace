...
export default function DockerComposeGenerator() {
  const [services, setServices] = useState([
    { name: "app", image: "node:18", ports: "3000:3000", env: "" },
  ]);
  ...
  return (
    <ToolLayout id="docker-compose-generator">
        ...
        {services.map((s, i) => (
          <div key={i} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-center">
            <input
              ...
              placeholder="service"
              className="p-2 bg-paper-dim/50 border border-border rounded text-sm font-mono text-foreground"
            />
            <input
              value={s.image}
            ...
            <input
              value={s.ports}
              onChange={(e) => updateService(i, "ports", e.target.value)}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <ToolButton onClick={addService} variant="secondary">
          Add Service
        </ToolButton>
