...
export default function AES() {
  ...
  const encrypt = async () => {
    try {
      ...
      const result = btoa(
        String.fromCharCode
          .apply(null, Array.from(iv) as number[])
          .concat(
            String.fromCharCode.apply(null, Array.from(new Uint8Array(encrypted)) as number[]),
          ),
      );
      setOutput(result);
    } catch {
  ...
  const decrypt = async () => {
    try {
      ...
      setOutput("Decryption failed — check input and key");
    }
  };
  ...
  return (
    <ToolLayout id="aes-encrypt-decrypt">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <ToolButton onClick={decrypt} variant="secondary">
          Decrypt
        </ToolButton>
      </div>
