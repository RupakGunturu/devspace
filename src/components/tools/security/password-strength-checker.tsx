...
export default function PasswordStrengthChecker() {
  ...
  const check = () => {
    ...
    } else details.push("✗ Less than 8 characters");
    ...
    } else details.push("✗ No lowercase letters");
    if (/[A-Z]/.test(password)) {
      score += 1;
      details.push("✓ Uppercase letters");
    } else details.push("✗ No uppercase letters");
    ...
    } else details.push("✗ No special characters");
    ...
    const crackTime =
      crackSec < 1
        ? "Instant"
        : crackSec < 60
          ? `${Math.round(crackSec)} seconds`
          : crackSec < 3600
            ? `${Math.round(crackSec / 60)} minutes`
            : crackSec < 86400
              ? `${Math.round(crackSec / 3600)} hours`
              : crackSec < 31536000
                ? `${Math.round(crackSec / 86400)} days`
                : `${Math.round(crackSec / 31536000)} years`;
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-1">
              {Array.from({ length: 7 }).map((_, i) => (
                <div
                  key={i}
                  className={`w-8 h-2 rounded-full ${i < result.score ? "bg-current " + result.color : "bg-paper-dim"}`}
                />
              ))}
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-3 bg-paper-dim/50 border border-border rounded-sm">
              ...
              <p className="font-mono text-lg font-bold text-foreground">{result.entropy} bits</p>
            </div>
            <div className="p-3 bg-paper-dim/50 border border-border rounded-sm">
              <span className="text-[10px] uppercase tracking-wider text-muted-foreground">
                Crack Time
              </span>
              <p className="font-mono text-lg font-bold text-foreground">{result.crackTime}</p>
            </div>
          </div>
