...
export default function RsaKeyGenerator() {
  ...
  const generate = async () => {
    ...
    setKeys({
      pub:
        "-----BEGIN PUBLIC KEY-----\n" +
        btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(pubRaw)) as number[]))
          .match(/.{1,64}/g)!
          .join("\n") +
        "\n-----END PUBLIC KEY-----",
      priv:
        "-----BEGIN PRIVATE KEY-----\n" +
        btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(privRaw)) as number[]))
          .match(/.{1,64}/g)!
          .join("\n") +
        "\n-----END PRIVATE KEY-----",
    });
      ...
      {keys && (
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-paper-dim/50 border border-border rounded-sm">
            ...
            <button
              onClick={() => navigator.clipboard.writeText(keys.pub)}
              className="text-xs text-muted-foreground hover:text-foreground px-2 py-1 rounded hover:bg-paper-dim"
            >
              Copy
            </button>
          </div>
          <pre className="p-3 bg-paper-dim/50 border border-border rounded-sm text-xs font-mono text-foreground overflow-auto max-h-32">
            {keys.pub}
          </pre>
          <div className="flex items-center justify-between p-3 bg-paper-dim/50 border border-border rounded-sm">
            <span className="text-xs text-muted-foreground">Private Key</span>
            <button
              onClick={() => navigator.clipboard.writeText(keys.priv)}
              className="text-xs text-muted-foreground hover:text-foreground px-2 py-1 rounded hover:bg-paper-dim"
            >
              Copy
            </button>
