...
const MAPPINGS: FormulaMapping[] = [
  {
    pattern: /(?:if|when)\s+(.+?)\s+then\s+(.+?)(?:\s+else\s+(.+))?$/i,
    airtable: "IF({$1}, {$2}, {$3})",
    notion: 'if(prop("$1"), $2, $3)',
    description: "Conditional IF/THEN/ELSE",
  },
  {
    pattern: /(?:sum|total|add)\s+(.+?)\s*(?:and|,|\+)\s*(.+)/i,
    airtable: "SUM({$1}, {$2})",
    notion: "add($1, $2)",
    description: "Sum two fields",
  },
  {
    pattern: /(?:average|avg)\s+(.+)/i,
    airtable: "AVERAGE({$1})",
    notion: "$1.map(current => current).reduce((a, b) => a + b, 0) / $1.length",
    description: "Average of a field",
  },
  {
    pattern: /count\s+(?:all\s+)?(?:rows?|records?|entries)\s*(?:where|where)\s+(.+)/i,
    airtable: "IF(COUNTA(FILTER({$1})) > 0, COUNTA(FILTER({$1})), 0)",
    notion: "filter(current.$1, true).length",
    description: "Count rows matching condition",
  },
  {
    pattern: /concat(?:enate)?\s+(.+?)\s+and\s+(.+)/i,
    airtable: 'CONCATENATE({$1}, " ", {$2})',
    notion: '$1 + " " + $2',
    description: "Concatenate two fields",
  },
  {
    pattern: /date\s+(?:diff|between)\s+(.+?)\s+and\s+(.+)/i,
    airtable: 'DATETIME_DIFF({$1}, {$2}, "days")',
    notion: 'dateBetween($1, $2, "days")',
  ...
  {
    pattern: /length\s+(?:of\s+)?(.+)/i,
    airtable: "LEN({$1})",
    notion: "length($1)",
    description: "Length of text field",
  },
  {
    pattern: /(?:upper|uppercase)\s+(.+)/i,
    airtable: "UPPER({$1})",
    notion: "upper($1)",
    description: "Convert to uppercase",
  },
  {
    pattern: /(?:lower|lowercase)\s+(.+)/i,
    airtable: "LOWER({$1})",
    notion: "lower($1)",
    description: "Convert to lowercase",
  },
  {
    pattern: /(?:trim|strip)\s+(.+)/i,
    airtable: "TRIM({$1})",
    notion: 'replace($1, " ", "")',
    description: "Trim whitespace",
  },
  {
    pattern: /contains?\s+(.+?)\s+(?:in|of)\s+(.+)/i,
    airtable: "IF(FIND({$1}, {$2}) > 0, TRUE(), FALSE())",
    notion: "contains($2, $1)",
    description: "Check if text contains substring",
  },
  {
    pattern: /(?:is empty|is blank)\s+(.+)/i,
    airtable: 'IF({$1} = "", TRUE(), FALSE())',
    notion: "empty($1)",
    description: "Check if field is empty",
  },
  {
    pattern: /(?:is not empty|is not blank)\s+(.+)/i,
    airtable: 'IF({$1} != "", TRUE(), FALSE())',
    notion: "!empty($1)",
    description: "Check if field is not empty",
  },
  {
    pattern: /round\s+(.+)\s+to\s+(\d+)\s*(?:decimals?|digits?)?/i,
    airtable: "ROUND({$1}, $2)",
    notion: "round($1, $2)",
    description: "Round to N decimals",
  },
  {
    pattern: /max(?:imum)?\s+(.+)/i,
    airtable: "MAX({$1})",
    notion: "max($1)",
    description: "Maximum value",
  },
  {
    pattern: /min(?:imum)?\s+(.+)/i,
    airtable: "MIN({$1})",
    notion: "min($1)",
    description: "Minimum value",
  },
  {
    pattern: /replace\s+(.+?)\s+with\s+(.+?)\s+in\s+(.+)/i,
    airtable: "SUBSTITUTE({$3}, {$1}, {$2})",
    notion: "replace($3, $1, $2)",
    description: "Replace text in string",
  },
  {
    pattern: /days?\s+(?:until|until)\s+(.+)/i,
    airtable: 'DATETIME_DIFF({$1}, TODAY(), "days")',
    notion: 'dateBetween($1, now, "days")',
    description: "Days until a date",
  },
  {
    pattern: /month\s+(?:of\s+)?(.+)/i,
    airtable: "MONTH({$1})",
    notion: "month($1)",
    description: "Extract month from date",
  },
  {
    pattern: /year\s+(?:of\s+)?(.+)/i,
    airtable: "YEAR({$1})",
    notion: "year($1)",
  ...
  return (
    <ToolLayout id="airtable-formula-helper">
      <ToolInput
        ...
        onChange={setInput}
        placeholder={
          "e.g. if status is done then completion date else empty\n\nUse field names in plain English. Reference: {FieldName}"
        }
      ...
      {result && (
        <div className="space-y-3">
          ...
          <div className="rounded-lg border-2 border-line bg-input-bg p-4">
            ...
            <pre className="whitespace-pre-wrap break-all font-mono text-sm text-input-text">
              {result.airtable}
            </pre>
          </div>
