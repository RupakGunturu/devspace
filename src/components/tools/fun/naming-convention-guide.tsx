...
const naming: Record<string, { rule: string; examples: string[] }> = {
  Variables: {
    ...
    examples: ["userName", "isActive", "totalPrice"],
  },
  Constants: {
  ...
  Classes: {
    rule: "Use PascalCase for classes and components",
    examples: ["UserService", "AppRouter", "UserProfile"],
  },
  Files: {
    rule: "Use kebab-case for file names",
    examples: ["user-service.ts", "app-router.tsx", "user-profile.tsx"],
  },
  "CSS Classes": {
    rule: "Use BEM or kebab-case for CSS classes",
    examples: ["user-card", "btn-primary", "nav__item--active"],
  },
  Private: {
    rule: "Prefix with underscore for private members",
    examples: ["_userId", "_fetchData()", "_isValid"],
  },
  Boolean: {
    rule: "Use is/has/can prefix for booleans",
    examples: ["isActive", "hasPermission", "canEdit"],
  },
  "Event Handlers": {
    rule: "Use handle/on prefix for event handlers",
    examples: ["handleSubmit", "onClick", "onChange"],
  },
};
          ...
          <div key={name} className="p-4 bg-paper-dim/50 border border-border rounded-sm">
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              {examples.map((e) => (
                <code
                  key={e}
                  className="px-2 py-1 text-xs font-mono bg-paper-dim border border-border rounded"
                >
                  {e}
                </code>
              ))}
