...
export default function BarcodeGenerator() {
  ...
  const generate = () => {
    const bars = input
      .toUpperCase()
      .split("")
      .map((c) => CODE128[c] || CODE128["0"])
      .flat();
    let svg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="' + bars.length * 3 + '" height="80">';
    let x = 10;
    bars.forEach((width, i) => {
      if (i % 2 === 0)
        svg += `<rect x="${x}" y="10" width="${width * 2}" height="50" fill="black"/>`;
      x += width * 2;
    });
  ...
  return (
    <ToolLayout id="barcode-generator">
      ...
      {output && (
        <div
          className="flex flex-col items-center gap-4 p-4 bg-white rounded-sm border border-border"
