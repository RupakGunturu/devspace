...
const REGIONS: Record<
  string,
  { label: string; brackets: TaxBracket[]; selfEmployment: number; deduction: number }
> = {
  US: {
    label: "United States",
    brackets: [
      ...
      {result && (
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-md border-2 border-line bg-input-bg p-3 text-center">
              <div className="font-mono text-xs text-muted">
                Deduction ({REGIONS[region].label})
              </div>
              <div className="mt-1 font-display text-xl font-extrabold text-input-text">
                ${fmt(REGIONS[region].deduction)}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-md border-2 border-dashed border-line p-3 font-mono text-xs text-muted">
            Estimates only \u2014 based on {REGIONS[region].label}{" "}
            {region === "US" ? "2024" : "current"} tax brackets. Consult a tax professional for
            actual filing.
          </div>
        </>
      )}
