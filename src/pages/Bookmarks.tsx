...
export default function Bookmarks() {
  ...
  const allItems = [
    ...favorites,
    ...savedTips.map((s) => ({ type: "tip" as const, slug: s.tipId, addedAt: s.savedAt })),
  ];

  const filtered = activeTab === "all" ? allItems : allItems.filter((f) => f.type === activeTab);

  const counts = {
    all: allItems.length,
    tool: favorites.filter((f) => f.type === "tool").length,
    tip: favorites.filter((f) => f.type === "tip").length + savedTips.length,
        ...
        {TABS.map((tab) => (
          ...
          >
            ...
            {counts[tab.key as keyof typeof counts] > 0 && (
              <span className="ml-1 opacity-60">{counts[tab.key as keyof typeof counts]}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-16 text-center">
          ...
          >
            ...
            {activeTab === "all"
              ? "content"
              : `${activeTab === "cheatsheet" ? "cheat sheets" : activeTab === "stack-breakdown" ? "stack breakdowns" : `${activeTab}s`} →`}
          </Link>
        </div>
      ) : (
          ...
          {filtered.map((fav) => {
                    ...
                    <StickerCard
                      icon={
                        ...
                        >
                          <ToolIcon
                            name={tool.icon}
                            className={CATEGORY_COLORS[tool.category]?.icon}
                          />
                        </div>
                      }
                      title={tool.name}
            ...
            if (fav.type === "tip") {
              ...
              return (
                <CursorHover label={tip.title} color="#eab308" key={fav.slug}>
                  <div className="relative">
                    <TipBookmarkButton tipId={tip.id} className="absolute right-2 top-2 z-10" />
                    <StickerCard icon={tip.icon} title={tip.title}>
            ...
            if (fav.type === "cheatsheet") {
              ...
              return (
                <CursorHover
                  label={sheet.title}
                  color={SHEET_COLORS[sheet.category]}
                  key={fav.slug}
                >
                  <div className="relative">
                    <BookmarkButton
                    ...
                    <StickerCard
                      icon={
                        ...
                        >
                          <ToolIcon
                            name={sheet.icon}
                            className={CATEGORY_COLORS_MAP[sheet.category]?.icon}
                          />
                        </div>
                      }
                      title={sheet.title}
